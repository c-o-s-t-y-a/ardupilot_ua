...
5. Якщо з throttle внизу і вправо arm не вдається і мотори не обертаються, найімовірніше, не пройдено pre-arm (передпольотна перевірка).
    ...
    - Якщо перевірка pre-arm не пройдена, перейдіть на [сторінку перевірок Pre-Arm](../safety/prearm-checks.md) і виправте проблему або вимкніть перевірку, перш ніж продовжити.
