# Custom Firmware Builder (конструктор власних збірок firmware)

> Оригінал: [ArduPilot Custom Firmware Builder](https://ardupilot.org/copter/docs/common-custom-firmware.html)

Завдяки внеску спільноти й зусиллям розробників ArduPilot швидко розвивається, і в ньому з'являється багато нових функцій. Проте не всім користувачам потрібна кожна функція. Залежно від своїх вимог і застосувань користувачі можуть захотіти прибрати певні функції з програми.

Для цього ми створили **Custom Firmware Builder** — вебзастосунок за адресою [custom.ardupilot.org](https://custom.ardupilot.org/). Він допомагає зменшити розмір firmware (прошивка), щоб вона легше вміщалася у flight controller (польотний контролер) з обмеженою flash (флеш-пам'ять). Це також допомагає тестувальникам: вони можуть тестувати конкретні функції, не замінюючи flight controller через брак місця.

Вихідний код конструктора власних збірок — [тут](https://github.com/ArduPilot/CustomBuild/).

## Як замовити збірку

Щоб замовити збірку, виконайте такі кроки:

1. Натисніть кнопку **'Add a build'** у верхньому правому куті головної сторінки.

    <img src="https://ardupilot.org/copter/_images/custom-firmware-build-server-addbuild.png" alt="custom-firmware-build-server-addbuild" width="450">

2. Виберіть — саме в такому порядку — тип апарата, версію і плату, для яких потрібно зібрати firmware.

    <img src="https://ardupilot.org/copter/_images/custom-firmware-build-server.png" alt="custom-firmware-build-server" width="450">

3. Виберіть функції, які потрібно включити у firmware.

    - Стандартні функції для плати/апарата буде вибрано автоматично.
    - Деякі функції залежать від інших, тож коли вибрано функцію, всі її залежності вибираються автоматично; але якщо згодом зняти вибір із функції, залежності автоматично не знімаються.
    - Можна вибрати набір функцій, який не вміститься на плату, — тоді збірка завершиться помилкою.

4. Натисніть кнопку **'Generate'**. Вас буде перенаправлено на головну сторінку, де з'явиться журнал збірки для щойно поданого запиту.
5. Коли збірка завершиться, її результат завантажиться автоматично. Також його можна завантажити вручну кнопкою **'Download'** поруч із вашим запитом у таблиці збірок. Кнопка стає активною лише після успішної збірки.
6. Розпакуйте завантажений архів. Серед розпакованих файлів буде файл firmware .apj та інші файли.
7. Встановіть файл .apj на autopilot (автопілот) за допомогою вашої GCS (наземна станція керування). У Mission Planner відкрийте Install Firmware і натисніть посилання «Load custom firmware».
...
Це поточна гілка розробки ArduPilot. Вона містить нові функції, покращення, прибирання коду тощо, але ще не пройшла релізний цикл бета-тестування. Вона пройшла альфа-тестування і ретельно перевірена набором автотестів Continuous Integration ArduPilot. Багато досвідчених користувачів вибирають її заради найновіших функцій, але в неї найвищий ризик щодо можливих помилок.

### Beta

Це реліз гілки розробки для широкого тестування і збору відгуків. Ризик серйозних проблем низький, але можливі незначні відхилення в поведінці. Зазвичай їх виправляють у наступних бета-релізах нового великого релізу, перш ніж оголосити код стабільним.

### Stable

Це повністю випущені версії коду. Для кожного номера великого релізу зазвичай є кілька малих релізів, що виправляють аномалії, знайдені під час бета-тестування. У цих релізів найнижчий ризик. Рекомендується використовувати реліз з найбільшим номером (великим/малим) у певній серії релізів, якщо не потрібна старіша версія.
...
## Скільки місця займають функції

Часто сервер власних збірок використовують, щоб отримати firmware з функціями, які прибрано зі стандартної firmware заради економії flash, але потрібні користувачу. Якщо збірка не завершується через брак flash, можна прибрати непотрібні функції/датчики, щоб звільнити місце для потрібних. Щоб полегшити цей вибір, у таблиці нижче показано, скільки flash звільняє вилучення функції (у байтах). Зверніть увагу: вилучення функції з цього списку автоматично вилучає всі функції, що від неї залежать.

> **Примітка.** Точна кількість байтів з часом дещо змінюється через зміни в коді, але ця таблиця достатньо точна, щоб вирішити, що варто вилучити для звільнення flash.

| Функція | PLANE | COPTER | ROVER |
|---|---:|---:|---:|
...
## Версії сервера збірок

Перш ніж розгортати нові функції на основному екземплярі застосунку, ми тестуємо їх на окремому тестовому екземплярі. Він доступний за адресою [custom-beta.ardupilot.org](https://custom-beta.ardupilot.org). Користуйтеся бета-функціями і надсилайте відгуки — це допоможе нам покращити застосунок.
